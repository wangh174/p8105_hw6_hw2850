...
From the plot, we can see that Albuquerque has the highest odd ratio for
solving homicides comparing male victims to female victims whereas New
York has the lowest odd ratio for solving homicides comparing male
...
Interested in investigating the relationship between mother’s weight
related variables (ppwt, ppbmi, wtgain, delwt) and baby’s birth weight
because mother’s pre-pregnancy weight, pre-pregnancy BMI, weight gain
during pregnancy and delivery weight could be factors of mothers’ health
condition and related to their newborns weight.
...
between ppwt, ppbmi, wtgain, delwt and bwt. But let’s try fit a linear
(MLR) model anyways.
...
```
    ...
    ##   <chr>          <dbl>     <dbl>     <dbl>     <dbl>
    ## 1 (Intercept)  2366.      54.4       43.5   0       
    ## 2 ppwt            9.15     0.698     13.1   1.46e-38
    ## 3 ppbmi         -29.9      4.44      -6.73  1.88e-11
    ## 4 wtgain         11.9      0.674     17.6   3.86e-67
    ## 5 delwt          NA       NA         NA    NA
...
From the graph, we can clearly see that the RMSEs for our linear model
are significantly higher than the MLR model and Interaction model.
Therefore, our model is not ideal. Between MLR model and Interaction
model, the latter one has lower rmse. Therefore, baby’s head
circumference, length, and sex are interacting with each other and the
Interaction model is better than the MLR model with length at birth and
